<template>
  <div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <form @submit.prevent="addCategory" class="max-w-md mx-auto my-10">
      <input v-model="newCategory.name" placeholder="Nom" required class="w-full mb-4 px-4 py-2 border border-gray-300 rounded-lg text-base font-sans" />
      <textarea v-model="newCategory.description" placeholder="Description" required class="w-full mb-4 px-4 py-2 border border-gray-300 rounded-lg text-base font-sans" rows="3"></textarea>
      <button type="submit" class="bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded-lg font-bold text-base w-full">Ajouter une catégorie</button>
    </form>
        ...
        <div class="content">
          <div v-if="category.id !== editedCategoryId">
            ...
            <div class="flex flex-col sm:flex-row justify-center sm:justify-end sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
              ...
              <button>
                <router-link :to="`/categories/${category.id}/`" class="bg-blue-500 hover:bg-blue-700 text-black py-2 px-4 rounded-lg font-bold text-white">Voir</router-link>
              </button>
            </div>
          </div>

          <form v-else @submit.prevent="updateCategory" class="flex flex-col items-center">
            <input v-model="editedCategory.name" placeholder="Nom" required class="w-full mb-4 px-4 py-2 border border-gray-300 rounded-lg text-base font-sans" />
            <textarea v-model="editedCategory.description" placeholder="Description" required class="w-full mb-4 px-4 py-2 border border-gray-300 rounded-lg text-base font-sans" rows="3"></textarea>
            <div class="flex flex-col sm:flex-row justify-center sm:justify-end sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
              <button type="submit" class="bg-green-500 hover:bg-green-600 py-2 px-4 rounded-lg font-bold text-white">Enregistrer</button>
              <button @click="cancelEdit" class="bg-gray-300 hover:bg-gray-400 py-2 px-4 rounded-lg font-bold text-white">Annuler</button>
            </div>
          </form>
        </div>
      </li>
